...
with open(sys.argv[1]) as csvFile:
    ...
    for row in reader:
        if row['Answer.WritingTexts'] not in mTurkParaphrases:
            mTurkParaphrases.append(row['Answer.WritingTexts'])
...
for queryBody in mTurkParaphrases:
    output[queryBody] = queryBody

outputFile = open('simulated_data_paraphrase/query_cluster.simulated.csv','w')
    ...
    if 'titles' in matchedCluster:
        for title in matchedCluster['titles']:
            ...
            if title != queryTitle:
                ...
                resultNum += 1
        selectedNum = randint(0, RESULTS_NUM_TO_DISPLAY)

        outputRows = [unicode(queryTitle).encode('utf-8'), unicode(queryBody).encode('utf-8'), unicode(selectedNum).encode('utf-8')]
